...
<body>
	<header> 
		...
		<% if (isLogin) { %>  
			<% if (grade == 0) { %>  
				<button class="theme-btn" onclick="location.href='/summat/sm/admin/memberList.jsp'">회원 관리</button>  
			<% } else if (grade == 1) { %>  
				<button class="theme-btn" onclick="location.href='/summat/sm/user/mypage.jsp'">마이페이지</button>  
			<% } %>  
			<button class="theme-btn" onclick="location.href='/summat/sm/user/logoutPro.jsp'">로그아웃</button>  
		<% } else { %>  
...
function login() {
  location.href="/summat/sm/user/loginForm.jsp";
}
